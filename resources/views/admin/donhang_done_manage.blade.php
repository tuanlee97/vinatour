@extends('admin.ADtemplate.ADmaster')
        ...
        <div class="card shadow mb-4">
          <div class="card-header py-3">
            ...
            Thông kê đơn đặt đã xử lí</div>
          <div class="card-body">
               <div align="right">
      <button type="button" name="delete" id="delete" class="btn btn-danger btn-sm">Xóa</button>
     </div>
     <br/>
             <div class="table-responsive " style="text-align: center;">
              <table class="table table-bordered donhang_done" id="dataTableDonHang_Done" width="100%" cellspacing="0">
                <thead>
                  <tr>
                    <th >Mã Tour</th>
                    <th>Mã khách hàng</th>
...
$(document).ready(function(){
 ...
 $('#dataTableDonHang_Done').DataTable({
  processing: true,
  serverSide: true,
  ajax:{
   url: "{{ route('donhang_done.index') }}",
  },
...
render:function(data,type,full,meta){
    return ' <span style="color:#10cc10;font-weight:bold">Đã xử lý</span>';
}},
   {data: 'thanhtoan',
...
$('.checkall').change(function() {
    var checkboxes = $(this).closest('.donhang_done').find(':checkbox');
...
$('.status:checked').each(function(){
                    ...
                    success:function(data)
                    {
                        ...
                        setTimeout(function(){
					     $('#confirmModal').modal('hide');
					       $('#dataTableDonHang_Done').DataTable().ajax.reload();
 ...
 $(document).on('click', '.details', function(){
   ...
   $('#thongtin').empty();
  $.ajax({
   url:"donhang_done/"+id_details+"/details",
   dataType:"json",
